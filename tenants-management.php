<?php
include('sessionchecker.php'); // Include session checker to ensure user is logged in
...
<head>
    ...
    <title> Apt-Ease - Tenants Management Page</title>
</head>
        ...
        <div class="wrapper-container">
            ...
            <div class="main-content">
                <div class="page-title">
                    ...
                    <p>Create, Delete, and Manage Tenants</p>
                </div>
                ...
                <div class="table_component" role="region" tabindex="0">
                    ...
                    function buildTable($result)
                    {
                        if (mysqli_num_rows($result) > 0) {
                            ...
                            echo "<th>Apartment No.</th><th>First name</th><th>Last name</th><th>Contact No.</th><th>Downpayment</th><th>Date added</th><th>Action</th>";
                            echo "</tr>";
                            echo "<br>";
                            //display the data of the tablecx
                            while ($row = mysqli_fetch_array($result)) {
                                echo "<tr class='cells'>";
                                echo "<td class = 'items'>" . $row['apartmentNo'] . "</td>";
                                echo "<td class = 'items'>" . $row['firstname'] . "</td>";
                                echo "<td class = 'items'>" . $row['lastname'] . "</td>";
                                echo "<td class = 'items'>" . $row['contactNo'] . "</td>";
                                echo "<td class = 'items'>" . $row['downpayment'] . "</td>";
                                echo "<td class = 'items'>" . $row['dateadded'] . "</td>";
                                echo "<td class= 'action'>";
                                echo "<button class='update-btn' onclick=\"window.location.href='update-tenant.php?apartmentNo=" . $row['apartmentNo'] . "'\">
                                ...
                                echo "<button class='delete-btn' onclick=\"confirmDelete('" . $row['apartmentNo'] . "')\">
                                <svg xmlns='http://www.w3.org/2000/svg' width='24' height='24' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'>
                                    <path stroke='none' d='M0 0h24v24H0z' fill='none'/>
                                    <path d='M4 7l16 0' />
                                    <path d='M10 11l0 6' />
                    ...
                    if (isset($_POST['btnsearch'])) {
                        $sql = "SELECT * FROM tbltenants WHERE apartmentNo LIKE ? OR firstname LIKE ? OR middlename LIKE ? OR lastname LIKE ? OR contactNo LIKE ? ORDER BY apartmentNo";
                        ...
                        $sql = "SELECT * FROM tbltenants ORDER BY apartmentNo";
                        if ($stmt = mysqli_prepare($link, $sql)) {
                            if (mysqli_stmt_execute($stmt)) {
                                $result = mysqli_stmt_get_result($stmt);
                                buildTable($result);
                ...
                <div id="updateModal" class="modal">
                    <div class="modal-content">
                        <form action="update-tenant.php" method="POST"></form>
                        <span class="close" onclick="closeModal('updateModal')">&times;</span>
                        <p>Are you sure you want to update this tenant?</p>
                ...
                <div id="deleteModal" class="modal">
                    <div class="modal-content">
                        ...
                        <form id="deleteForm" action="delete-tenant.php" method="POST">
                            <input type="hidden" name="txtapartmentNo" id="deleteApartmentNo">
...
<script>
    ...
    function openUpdateModal(apartmentNo) {
    ...
    function confirmDelete(apartmentNo) {
